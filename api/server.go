...
func (s *Server) handleGetSupportedModels(c *gin.Context) {
	...
	// 若未配置custom模型，但本地存在 trader/config.json 且声明了自定义模型，则补充一个“custom”供前端选择
	hasCustom := false
	for _, m := range models {
		if m.Provider == "custom" || m.ID == "custom" {
			hasCustom = true
			break
		}
	}
	...
	if !hasCustom {
		// 检查 trader/config.json 是否存在并为自定义模型
		type traderCustomModel struct {
			AIModel         string `json:"ai_model"`
			CustomAPIURL    string `json:"custom_api_url"`
			CustomModelName string `json:"custom_model_name"`
		}

		if _, err := os.Stat("trader/config.json"); err == nil {
			if data, err := os.ReadFile("trader/config.json"); err == nil {
				var tcm traderCustomModel
				if json.Unmarshal(data, &tcm) == nil {
					if strings.ToLower(tcm.AIModel) == "custom" && tcm.CustomAPIURL != "" && tcm.CustomModelName != "" {
						models = append(models, &config.AIModelConfig{
							ID:              "custom",
							UserID:          "default",
							Name:            "Custom Model",
							Provider:        "custom",
							Enabled:         true, // 可选：为便于选择，标记为可用
							CustomAPIURL:    tcm.CustomAPIURL,
							CustomModelName: tcm.CustomModelName,
						})
					}
				}
			}
